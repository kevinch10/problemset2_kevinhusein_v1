...
export const CompareTowns: React.FC<CompareTownsProps> = ({
  ...
  const statsA: TownSummary = useMemo(() => {
    return computeTownSummary(townA, transactions);
  }, [townA, transactions]);

  const statsB: TownSummary = useMemo(() => {
    return computeTownSummary(townB, transactions);
  }, [townB, transactions]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center gap-3 sm:gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <select
                ...
                className="w-full h-12 px-3.5 pr-8 bg-slate-50 border border-slate-300 rounded-xl text-sm sm:text-base font-bold text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-slate-900 transition-colors cursor-pointer"
              >
                {SELECTABLE_TOWNS.map((town) => (
                  <option key={`town-a-${town}`} value={town}>
                    {town}
                  </option>
                ))}
              </select>
          ...
          <div className="flex justify-center md:pt-6">
            <button
              ...
              type="button"
              onClick={swapTowns}
              className="inline-flex items-center justify-center gap-1.5 px-4 h-12 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 text-xs font-bold transition-colors cursor-pointer min-h-[44px]"
              title="Swap Towns"
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              >
                {SELECTABLE_TOWNS.map((town) => (
                  <option key={`town-b-${town}`} value={town}>
                    {town}
                  </option>
                ))}
              </select>
